...
class _ClipState extends State<Clip> {
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        ...
                        IconButton(
                          ...
                          onPressed: () {
                            showSearch(
                                context: context, delegate: ContSearch());
                          },
                        )
                      ],
                    ),
  ...
  Future<Null> handleRefresh() async {
    ...
    setState(() {
      ...
      print(
          "TotalCases: ${listData[1]['totalCases'].toString()} \nSecond: ${listData[1]['country']}");
    });
    // Completer<Null> completer = new Completer<Null>();
  }

  Widget _countryList(double scWidth) {
                ...
                onTap: () {
                  setState(() {
                    ...
                    Navigator.pushNamed(context, '/detail', arguments: {
                      ...
                      'totalRecovered': listData[index + 2]['totalRecovered'],
                    ...
                    children: <Widget>[
                      ClipOval(
                        child: CachedNetworkImage(
                          imageUrl: listData[index + 2]['countryInfo']['flag'],
                          width: 60,
                          height: 60,
                          fit: BoxFit.cover,
                        ),
...
class ContSearch extends SearchDelegate<String> {
  ...
  Widget buildSuggestions(BuildContext context) {
    ...
    for (int i = 2; i < listData.length; i++) {
      cou.add(listData[i]['country']);
    }
    final suggestionList = query.isEmpty
        ? recentCountries
        : cou.where((p) => p.startsWith(query)).toList();
    return ListView.builder(
